 ...
 */
package com.carmozo.driverapp.Traccar;
...
public class TrackingController implements PositionProvider.PositionListener, NetworkManager.NetworkHandler {
    ...
    public TrackingController(Context context) {
        ...
        String keyProvider = C.TraccarParams.TRACCAR_KEY_PROVIDER;
        if ( keyProvider.equals("mixed")) {
        ...
        address = C.TraccarConnParam.TRACCAR_CONN_PARAM_ADDR;
        port = C.TraccarConnParam.TRACCAR_CONN_PARAM_PORT;

        PowerManager powerManager = (PowerManager) context.getSystemService(Context.POWER_SERVICE);
        wakeLock = powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, getClass().getName());
        Log.v(TAG, "[Guru]:" + "[" + address +"][" + port +"]");
    }
    ...
    public void start() {
        ...
        Log.v(TAG, "[Guru] : Start Tracking controller");
    ...
    private void send(final Position position) {
        ...
        Log.v(TAG, "[Guru]:" + request);
        RequestManager.sendRequestAsync(request, new RequestManager.RequestHandler() {
            @Override
            public void onComplete(boolean success) {
                if (success) {
                    Log.v(TAG, "[Guru]Send:" + success);
                    delete(position);
                } else {
                    delete(position); //Guru-- Testing purpose. Should not be delete.
                    //StatusActivity.addMessage(context.getString(R.string.status_send_fail));
                    Log.v(TAG, "[Guru]Send:failed");
                    //retry();
                }
                unlock();
            }
